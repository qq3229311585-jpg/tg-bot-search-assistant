...
- [x] 运行完整测试、编译检查、差异检查和自检脚本。
- [x] 更新代码知识图谱，记录接口和数据流变化。
...
## Phase 5: 全板块日报保留与轮换（进行中）

- [x] 盘点原有板块：确认天气/GitHub 原生工具存在，Steam/代理/Hacker News/汇率/行情等依赖外部 `today_report.txt`。
- [x] 新增日报板块注册表，区分快照板块与事件板块，预留可注册采集器接口。
- [x] 为同板块冷却、跨板块独立历史、重复跳过和 legacy 文本保留补充测试。
- [x] 接入构建脚本的全板块策略与状态元数据。
- [x] 完成配置/部署文档、全量回归和代码复审。
...
## Phase 7: 远端部署（2026-07-14）

- [x] 检查远端 Debian、旧服务、数据目录和 systemd 状态。
- [x] 备份旧环境/代码/服务文件/运行数据，并同步 9931241 版本。
- [x] 创建专用 `tgbot` 用户、收紧代码/数据/环境文件权限，安装 Bot 与日报 timer。
- [x] 修复 systemd `PYTHONPATH`，确保服务加载部署目录而非服务器全局旧模块。
- [x] 验证 Bot、`/health`、`/readyz`、未授权 API、日报生成和 Telegram 推送。
