...
- [ ] 为结构化回复契约写失败测试：结论、关键依据、行动建议、来源四段；闲聊和工具结果保持兼容。
- [ ] 实现统一结构化渲染与长度/降级策略，不泄露内部推理。
- [ ] 将搜索回答和日报摘要接入结构化渲染，保留原始审计字段。
- [ ] 运行相关单元测试并重构重复逻辑。

### Phase 3: 日报事件去重与热度排序（TDD）

- [ ] 为事件指纹、跨天去重、同事件更新合并、热度排序写失败测试。
- [ ] 添加候选标准化、指纹生成、热度分数和多样性配额函数。
- [ ] 扩展日报状态存储，记录最近发布指纹、首次/最近出现时间、来源与分数。
- [ ] 更新日报采集/编排，优先新鲜高热事件，来源不足时明确降级。

### Phase 4: 文档、部署与验证

- [ ] 更新环境变量、日报格式、数据迁移和运维说明。
