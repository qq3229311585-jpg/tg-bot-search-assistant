...
def _fmt_evidence_appendix(facts_json, limit_facts=8, limit_evidence=2):
    ...
    for tr in (tool_results or []):
        ...
        lines.append(f"[素材{idx}] 搜索结果（{tr.get('tool','')}）")
...
def _source_priority(entry):
    ...
    if domain in ("wikipedia.org", "en.wikipedia.org", "zh.wikipedia.org"):
        score -= 3
    if domain in ("web_search", "serper_search"):
        score += 2
    if "official" in title or "官网" in title or "学校概况" in title:
        score -= 2
    return score
...
def _fmt_source_index_brief(source_index, limit_sources=8):
    seen = set()
    lines = []
    for entry in sorted(source_index or [], key=_source_priority):
...
def write_ai(user_text, fact_list, history_context=None, reject_feedback="", facts_json=None,
             source_index=None, verifier_checks=None):
    """
    第三层：写作 AI。
    ...
    evidence_appendix = _fmt_evidence_appendix(facts_json)
    ...
    _len_hint = _extract_length_constraint(user_text)
    if _len_hint:
        lo, hi = _len_hint
        user_content += (
