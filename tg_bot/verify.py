...
from tg_bot.prompts import _SYS_VERIFIER
...
def verify_reply(reply, fact_list, user_text="", attempt=0, source_index=None,
    ...
    if source_index:
        ...
        seen = set()
        ordered_sources = []
        for entry in sorted(source_index, key=_source_priority):
        ...
        for i, entry in enumerate(ordered_sources[:12]):
            ...
            content = (entry.get("full_content") or "")[:1500].strip()
    ...
    if fact_block:
        user_msg = (
            ...
            f"【原始素材】\n{source_block}\n"
            f"【待审回复】\n{reply}\n\n"
            "【强制要求】逐句核查待审回复。每个具体事实必须能映射到某个 F 编号或直接API证据；"
            "事实清单只有摘要、证据定位不到、或回复扩展出证据外细节时，判 REJECT。"
        )
    elif source_block:
        user_msg = (
            f"【用户原始问题】\n{user_text}\n\n"
